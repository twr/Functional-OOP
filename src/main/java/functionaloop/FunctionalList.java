...
public abstract class FunctionalList<T> {
	...
	public static <T> FunctionalList<T> cons(final T head,
			final FunctionalList<T> tail) {
	...
	private static <T> FunctionalList<T> fromArray(final T[] items,
			final int start, final int end) {
		return start == end ? FunctionalList.<T> nil() : cons(items[start],
	...
	public <U> FunctionalList<U> map(final Function<T, U> mapping) {
		return isEmpty() ? FunctionalList.<U> nil() : cons(mapping
				.apply(head()), tail().map(mapping));
	}

	public abstract T find(Function<T, Boolean> function);
	...
	public static final class Cons<T> extends FunctionalList<T> {
		...
		@Override
		public T find(Function<T, Boolean> function) {
			return function.apply(head) ? head : tail.find(function);
		}
	}
